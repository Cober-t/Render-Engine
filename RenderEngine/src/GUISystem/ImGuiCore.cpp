...
namespace Cober {
	...
	bool dockspaceOpen = true, dockspaceEnable = true;
	...
	void GUI::Init(const Unique<Window>& window) {
		...
		SDL_Window* _window = static_cast<SDL_Window*>(window->GetWindow());
		SDL_GLContext _context = static_cast<SDL_GLContext>(window->GetContext());
	...
	void GUI::Render(const Ref<Framebuffer>& framebuffer) {
		...
		if (framebuffer != nullptr)		// Viewport
		{
